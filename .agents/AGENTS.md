...
1. Run the build script and collect `OndeFramework.xcframework.zip` + checksum.
2. Create a GitHub Release on the `onde` repo and attach the zip.
3. In `onde-swift/Package.swift`, update the `.binaryTarget` to use the new `url:` and `checksum:`.
4. Commit, tag (`0.x.y`), and push `onde-swift`.
...
- **`OndeFramework`** — `.binaryTarget` pointing at `OndeFramework.xcframework` (local path during dev, remote URL+checksum for distribution).
...
- **`Package.swift` binary target path** must point to the correct `.xcframework` location: local path during development, remote URL+checksum for published releases.
...
4. Upload artifacts: zip, checksum, version, generated `onde.swift`.
5. On release: attach zip and checksum to the GitHub Release.

### Release Process (end-to-end)

1. Bump version in `onde/Cargo.toml`.
2. Tag and push `onde` repo → CI builds and attaches `OndeFramework.xcframework.zip`.
3. In `onde-swift`, update `Package.swift` with new `url:` and `checksum:`.
4. Commit, tag, push `onde-swift` → Swift Package Index indexes it automatically.
