...
Triggered on **semver tag push** (e.g. `git tag 0.2.0 && git push origin 0.2.0`) or `workflow_dispatch`. Runs on `macos-15`.

Steps:
1. Install stable Rust (`1.92.0`) with iOS/macOS targets.
2. Install nightly Rust with `rust-src` component (tvOS uses `-Z build-std`, no `rustup target add` needed).
3. Run `build-swift-xcframework.sh`.
4. Read `version.txt` and `OndeFramework.checksum.txt` into step outputs.
5. Validate tag matches `Cargo.toml` version (fails fast on mismatch).
...
9. On tag push: rewrite the `.binaryTarget` in `onde-swift/Package.swift` with the release download URL and computed checksum.
10. On tag push: copy the freshly generated `onde.swift` into `onde-swift/Sources/Onde/`.
...
2. Commit, tag, push:
   ...
   ```
3. CI builds the XCFramework, creates a GitHub Release, attaches assets, then **automatically** rewrites `onde-swift/Package.swift` with the correct `url:` + `checksum:`, commits, tags, and pushes `onde-swift`.
4. Swift Package Index picks up the new `onde-swift` tag automatically — no manual step needed.
5. `cargo publish` to crates.io.

> **Note:** Steps 3–4 are fully automated. Manual edits to `onde-swift/Package.swift` are never needed. Only touch it when doing local development (use the `path:` form documented in the file header).
