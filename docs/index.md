---
title: "Onde"
description: "On-device AI inference for every platform. Run LLMs, speech-to-text, and image generation locally — no cloud, no latency, no data leaving the device."
---

# Onde

**On-device AI inference for every platform.**

Run LLMs, speech-to-text, and image generation locally — no cloud, no latency, no data leaving the device. Onde wraps [mistral.rs](https://github.com/EricLBuehler/mistral.rs) for LLM and image generation, and [whisper.cpp](https://github.com/ggerganov/whisper.cpp) for speech-to-text, with a unified API that handles model discovery, HuggingFace Hub downloads, cache management, and GPU acceleration across every platform.
...
**One dependency.** Add `onde` and get LLM inference, speech-to-text, image generation, HuggingFace cache management, and HF token resolution in a single crate.
