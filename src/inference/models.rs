pub const BFL_FLUX_1_SCHNELL: &str = "black-forest-labs/FLUX.1-schnell";
pub const GOOGLE_GEMMA_3N_E2B_IT: &str = "google/gemma-3n-E2B-it";
pub const GOOGLE_GEMMA_3N_E4B_IT: &str = "google/gemma-3n-E4B-it";
pub const MICROSOFT_PHI_35_MINI_INSTRUCT: &str = "microsoft/Phi-3.5-mini-instruct";
...
pub const SUPPORTED_MODELS: &[&str] = &[
    ...
    MICROSOFT_PHI_35_MINI_INSTRUCT,
    BARTOWSKI_QWEN25_1_5B_INSTRUCT_GGUF,
    BARTOWSKI_QWEN25_3B_INSTRUCT_GGUF,
    BARTOWSKI_QWEN25_CODER_1_5B_INSTRUCT_GGUF,
...
pub struct SupportedModelInfo {
    /// The full HuggingFace model identifier, e.g. "black-forest-labs/FLUX.1-schnell".
    pub id: &'static str,
...
pub const SUPPORTED_MODEL_INFO: &[SupportedModelInfo] = &[
    SupportedModelInfo {
        id: BFL_FLUX_1_SCHNELL,
        name: "FLUX.1 Schnell",
        org: "Black Forest Labs",
        description: "Fast, high-quality image generation model optimised for speed.",
        // Only counting flux1-schnell.safetensors (23.78 GB) — the single-file
        // model weight that mistral.rs actually downloads for FluxOffloaded.
        expected_size_bytes: 23_782_506_688,
    },
    SupportedModelInfo {
        id: GOOGLE_GEMMA_3N_E2B_IT,
        name: "Gemma 3n E2B",
        org: "Google",
        description: "Compact prompt enhancer model — lightweight and fast on macOS.",
        // Sum of all repo files: 3 safetensor shards (3.08 + 4.98 + 2.82 GB)
        // + tokenizer.json (33 MB) + tokenizer.model (4.7 MB) + configs, etc.
        // Total ≈ 10.92 GB
        expected_size_bytes: 10_922_539_343,
    },
    SupportedModelInfo {
        id: GOOGLE_GEMMA_3N_E4B_IT,
        name: "Gemma 3n E4B",
        org: "Google",
        description:
            "Balanced prompt enhancer model — good quality-to-size ratio for macOS desktop.",
        // Sum of all repo files: 4 safetensor shards (3.08 + 4.97 + 4.99 + 2.66 GB)
        // + tokenizer.json (33 MB) + tokenizer.model (4.7 MB) + configs, etc.
        // Total ≈ 15.74 GB
        expected_size_bytes: 15_743_647_548,
    },
    SupportedModelInfo {
        id: MICROSOFT_PHI_35_MINI_INSTRUCT,
        name: "Phi-3.5-mini",
        org: "Microsoft",
        description:
            "Strongest prompt enhancer quality, larger memory footprint (~2.8 GB with Q4K).",
        // Sum of all repo files: 2 safetensor shards (4.97 + 2.67 GB)
        // + tokenizer.json (1.8 MB) + tokenizer.model (500 KB) + configs, etc.
        // Total ≈ 7.64 GB
        expected_size_bytes: 7_644_702_484,
    },
    SupportedModelInfo {
        id: BARTOWSKI_QWEN25_CODER_1_5B_INSTRUCT_GGUF,
        name: "Qwen 2.5 Coder 1.5B (GGUF)",
        org: "Qwen / Alibaba",
        description:
            "Dedicated coding model — same size as the general 1.5B (~941 MB) but trained \
             on 5.5T tokens of code and math. Strongly preferred for sigit. \
             Ideal for iOS & Android.",
        // Qwen2.5-Coder-1.5B-Instruct-Q4_K_M.gguf from bartowski repo.
        expected_size_bytes: 986_048_768,
    },
    SupportedModelInfo {
        id: BARTOWSKI_QWEN25_CODER_3B_INSTRUCT_GGUF,
        name: "Qwen 2.5 Coder 3B (GGUF)",
        org: "Qwen / Alibaba",
        description: "Best on-device coding quality for macOS desktop (~1.93 GB). \
             Trained on 5.5T tokens of code and math data. Not recommended for iOS.",
        // Qwen2.5-Coder-3B-Instruct-Q4_K_M.gguf from bartowski repo.
